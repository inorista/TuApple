...
class CustomerController extends Controller {
  public function home() {
    ...
    $newprods = ProductDAO::selectTopSale(4);
    $hotprods = ProductDAO::selectTopHot(4);
    $newprod2 = ProductDAO::selectTopNew(4);
    $data = array('cates' => $cates, 'newprods' => $newprods, 'newprod2' => $newprod2, 'hotprods' => $hotprods);
    ...
    if (isset($_POST['btnSubmit'])) { // POST
      ...
      if ($dbCust != null) {
        ...
        if ($newID > 0) {
          ...
          $content .= 'http://localhost:3000/public/customer/verify?id=' . $newID . '&token=' . $token;
  ...
  public function checkout() {
    if (Session::get('customer') != null) {
      ...
      MyUtil::showAlertAndRedirect('Bạn đã đặt hàng thành công!', 'home');
    } else {
